            ...
            const pratoHtml = `
                <div class="col-lg-6">
                    <div class="d-flex align-items-center">
                        <img class="flex-shrink-0 img-fluid rounded" src="${prato.imageUrl || 'img/placeholder.jpg'}" alt="${prato.name}" style="width: 80px;">
                        <div class="w-100 d-flex flex-column text-start ps-4">
                            <h5 class="d-flex justify-content-between border-bottom pb-2">
                                <span>${prato.name}</span>
                                <span class="text-primary">R$${prato.price.toFixed(2)}</span>
                            </h5>
                            <small class="fst-italic">${prato.description}</small>
                        </div>
                    </div>
                </div>
            `;
